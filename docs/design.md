...
ブラウザテストは当初「46 行の本文 + 見出し + 段落」と決め打ちにして、avoid 無しなら見出しが 1 ページ目末尾に取り残されることを前提にしていた。これが **Firefox の CI で落ちた**（0.4.0 公開後に発覚）。1 ページに入る行数はブラウザの行の高さの丸め方で変わり、Firefox では 46 行の時点で見出しがすでに 2 ページ目へ行っていたため、前提そのものが成り立たなかった（ライブラリの不具合ではない）。行数の決め打ちをやめ、**1 行の高さをブラウザ自身に測らせて境界の行数を求め**、その前後 3 行を掃く形に書き直し、**「avoid を付けたらどの行数でも見出しと次の段落が同じページに載る」**ことと、**「avoid 無しでは少なくとも 1 つの行数で離れる」**ことの 2 つを見るようにした。前者が機能の性質そのもので、後者が「avoid が効く状況を実際に通っている」ことの担保になる。Paginator の joins を無効化する変異を入れて、「46 行: avoid ありなら見出しと次の段落が同じページ」という名前つきで失敗することを確認済み。

なお最初の直しでは 43〜50 行と範囲を決め打ちにしたが、これも **Firefox で落ちた**（範囲内に境界が来なかった）。1 ページに入る行数は行ボックスの高さ次第で、Chromium では 21.328px / 47 行だが他ブラウザでは違う。測ってから掃くのが正しい。境界の行数と各行数での結果はテストの annotation に出すようにしたので、また落ちたときに数字が追える。
