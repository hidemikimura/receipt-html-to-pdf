...
- 作成者: 木村英実（ecx Inc.）
- ステータス: **0.4.0 リリース準備中**（0.1.0 / 0.2.0 / 0.2.1 / 0.3.0 を公開済み）。API は 1.0.0 まで変更の可能性あり
- ライブラリ名: **Receipt html to pdf**
- npm パッケージ名: `@hidemikimura/receipt-html-to-pdf` — 0.1.0（2026-09-18）、0.2.0（シャドウ DOM・18 章）、0.2.1（右端が切れる不具合の修正・19 章）、0.3.0（描画と大きな文書・20〜25 章）を公開済み。次は 0.4.0（リンク注釈としおり・26 章）
...
v1.x 候補（当初の私案）: `break-before/after: avoid`、`background-repeat`、グラデーション、SVG のベクター変換、GSUB（tabular-nums）、縦書き、リンク注釈、ストリーミング出力。

→ このうち `break-before/after: avoid`（20 章）・グラデーション（21 章）・SVG（22 章）・GSUB（23 章）・`background-repeat`（24 章）は対応済み。ストリーミング出力は 25 章の測定で見送り、代わりに進捗通知とメモリ解放を実装した。リンク注釈としおりも 26 章で対応済み。

**縦書きは見送り（2026-09-20 判断）。** 適格請求書（インボイス）の記載事項は横書きの表で組むのが実務上の標準で、縦書きの帳票需要は小さい。一方で実装は走査・計測・ページ分割のすべてに及ぶ（文字の進行方向が変わるため、ベースラインの求め方、行の扱い、ページ境界の判定、`vert` / `vrt2` によるグリフ置換がすべて別物になる）。**必要になった時点で改めて調査から始める。** それまでは `writing-mode: vertical-*` に `unsupported-css` の警告を出す現状のままとする。
...
## 19. 右端が切れる不具合（2026-09-20）

`examples/cdn.html` で作った PDF の右端が切れる、という報告から見つかった。原因は 2 つ重なっていた。

1. **サンプルの CSS の誤り**: `#receipt` に `width: 180mm` と `padding` / `border` を書きながら `box-sizing: border-box` を付け忘れていた。A4・余白 15mm の本文領域はちょうど 180mm なので、その分はみ出す。他のサンプルとフィクスチャには付いていた。
2. **本体の不具合**: iframe に差し込むリセット（`html,body{margin:0;padding:0}`）を `buildHtml()` が収集したスタイルより**前**に置いていたため、ページ側の `body { margin: 24px }` に同詳細度で負けていた。内容が右へずれ、同じ幅だけ右端が切れる。ページ側が body にマージンを持つのはごく普通なので、影響範囲は広い。リセットを `!important` にして打ち消すようにした（用紙の余白は `options.page.margin` だけが決める、という設計を明示する意味もある）。

あわせて `warnIfTooWide()` を足し、`scrollWidth` が本文領域より広いときに `other` の警告をはみ出し量つきで出すようにした。黙って切れるのが一番気づきにくいため。フィクスチャ 3 種（回転した「再発行」透かしを含む variant を含む）で誤検知が出ないことを確認している。

検証: ブラウザテストを 2 件追加（はみ出し警告、body マージンが持ち込まれないこと）。既存の画素差分テストが通ったので、リセットの変更で描画位置がずれていないことも確認できた。

## 20. `break-before/after: avoid` 対応（2026-09-20）

v1.x 候補の先頭として実装した。見出しがページ末尾に取り残されるのを防ぐ、実務で一番効く改ページ制御。

**モデル**: 既存の「アトム（跨いではいけない縦範囲）」では表せない。avoid が禁じるのは箱 A と箱 B の**間**に境界を置くことだけで、A の中や B の中で切るのは許されるからだ。そこで Join（`{start, end, pullTo}`）という別の概念を足した。`start` = A の下端、`end` = B の上端、`pullTo` = A の上端。境界がこの範囲に落ちたら `pullTo` まで戻す。

**隣の箱の探し方**（`walker/walk.js`）: `nextBoxAfter()` / `prevBoxBefore()` は、el の子孫を飛ばして文書順で次（前）に現れる箱を返す。兄弟が無ければ親をさかのぼるので、`<section>` の最後の見出しに `break-after: avoid` を書いても次の `<section>` と結びつく。`edgeBoxIn()` が `display: contents` / `inline` / 高さ 0 の要素を透かして実際の箱まで降りる。

**半行分のずれ**（ここで一度ハマった）: 境界を「B の上端」で判定すると発火しない。境界はアトム（行）の上端まで戻されるが、行ボックスは `line-height` の半行分だけ箱の上端より下にあるため、B の上端より数 px 下で止まるからだ。実測で B の上端が 1002.42、最初の行が 1004.42、境界が 1004.42 になり、`[1002.42, 1002.42]` にかからなかった。Paginator 側で各 Join について「B の上端以降にある最初のアトムの上端」を求め、そこまでを禁止範囲（`limit`）とすることで解決した。

**諦める条件**: 結んだ範囲がページ容量より大きいとき（戻しても同じ位置で切ることになる）と、戻し先がページ先頭以前のとき（空ページになる）は、avoid を無視して普通に分割する。連鎖（見出しが 2 つ続くなど）は、アトムと同じ `while (moved)` ループの中で処理するので自然に遡る。

検証: 単体テスト 6 件（発火・非発火・連鎖・諦める 2 条件）とブラウザテスト 1 件。minify バンドルは gzip 19.9KB → 20.3KB。

ブラウザテストは 0.4.0 公開後の CI（Firefox）で 3 回落ちた。**原因はライブラリではなくテストの作り**で、順に次のとおり。

1. 「46 行の本文 + 見出し + 段落」と決め打ちし、avoid 無しなら見出しが 1 ページ目末尾に残ることを前提にした → Firefox ではその行数で既に見出しが 2 ページ目にあった。
2. 行数を 43〜50 で掃く形にした → Firefox では範囲内のどの行数でも分離しなかった。
3. 1 行の高さをブラウザに測らせて境界を求め、その前後を掃く形にした → Firefox も境界を 46 行と算出したのに、やはり分離しなかった。

3 で「行の高さは一致しているのに結果が違う」と分かり、**このテストだけ渡す CSS に `@font-face` を書き忘れていた**ことに行き着いた。計測用 iframe にフォントが無いのでブラウザごとの代替フォントで測られ、行ボックスの中での**グリフ矩形（= 行アトム）の位置と高さ**がブラウザで変わる。境界を跨ぐアトムが「続く段落の行」ではなく「見出しの行」になれば、境界は見出しの上へ繰り上がり、avoid 無しでも見出しが段落と一緒に次ページへ行く。Firefox で分離しなかったのはこれ。

最終形は、`@font-face` を渡したうえで**行数にも行の高さにも依存しない**構成にした。詰め物 700pt + 見出し 40pt = 740pt までが 1 ページ目（本文領域 756.85pt）に入り、続く段落 740〜780pt が境界を跨ぐので、境界は必ず見出しと段落の間へ繰り上がる。すべて pt の実寸なのでフォントに左右されない。Paginator の joins を無効化する変異を入れて、「avoid ありなら見出しと続く段落が同じページ」で失敗することを確認済み。

**教訓**: `stylesheets` を明示するテストでは `@font-face` も必ず渡す。渡さないと計測だけ別フォントになり、ブラウザ差が入り込む（ライブラリ利用者が踏むのと同じ落とし穴）。同じ抜けが他の 6 件のブラウザテストにもあったので、`FONT_FACE` 定数にまとめて全部に渡すようにした（文字を一切描かない 1 件を除く）。画素差分のテストでは、比較元のスクリーンショットは親文書のフォントで描かれるのに変換側は代替フォントで測っていたことになり、差分が余計に出る状態だった。

未対応のまま: `orphans` / `widows`（行数単位の制御）。

## 21. `linear-gradient` 対応（2026-09-20）

ラスタライズして画像として埋めるのではなく、**PDF の軸シェーディング（ShadingType 2）** として出す。ベクターのままなので拡大しても滑らかで、ファイルも軽い。

**解析**（`walker/gradient.js`）: 入力は computed style なので、色は `rgb()` / `rgba()`、角度は `deg` に正規化済みで、パーサは素直に書ける。角度から CSS 仕様どおりグラデーション線の長さ `|W·sinθ| + |H·cosθ|` を求め、箱の中心を通る線分にする。CSS の 0deg は上向き・時計回り、こちらの座標系は y 下向きなので、方向ベクトルは `(sinθ, -cosθ)`。色止めは `%` / `px` / 位置省略（前後の確定位置の間を等分）/ 二重指定（`red 0% 40%`）を扱い、最後に単調化する（前の位置を下回ったら揃える）。`in oklab` などの補間指定は無視して sRGB で近似する。

**PDF 化**（`pdf/shading.js`）: 色止めの各区間を Type 2（指数補間、N=1 = 線形）関数にし、3 つ以上なら Type 3（継ぎ合わせ）でつなぐ。`Extend [true true]` で線分の外側も端の色で埋める。

**アルファ**: PDF のシェーディングは色しか持てないので、色止めごとにアルファが変わる場合は**輝度ソフトマスク**を使う。同じ形の DeviceGray シェーディング（値 = アルファ）を描くフォーム XObject を作り、`/SMask << /S /Luminosity /G <form> /BC [0] >>` の ExtGState で適用する。要素から継承した `opacity` はマスクのグレー値に掛け込んでおく。全色止めのアルファが同じなら従来どおり `ca` で済ませる。

**ページをまたいでも作り直さない工夫**: シェーディングの座標は用紙座標ではなく**箱ローカルの CSS px**（左上原点・y 下向き）で持ち、描画時に `cm` で `[0.75, 0, 0, -0.75, X(box.x), Y(box.y)]` と写す。こうすると同じシェーディングオブジェクトを全ページで使い回せる。ソフトマスクの `gs` は `cm` の**後**に置く（フォーム XObject は gs 実行時の座標系で評価されるため、マスクと本体の座標系が揃う）。

検証: パーサの単体テスト 14 件と、ブラウザテスト 1 件（4 種類のグラデーションを変換し、同じ HTML のブラウザ描画と画素差分。差分はテキストのアンチエイリアスだけで、グラデーション面はきれいに一致した）。minify バンドルは gzip 20.3KB → 21.8KB。

未対応のまま: `repeating-linear-gradient`、`radial-gradient`（ShadingType 3 + 楕円は変換行列が要る）、`conic-gradient`（PDF に直接の表現が無い）、複数背景レイヤー。

## 22. インライン SVG のベクター変換（2026-09-20）

これまでインライン `<svg>` は走査対象外で、`<img src="x.svg">` にしてラスタライズするしかなかった。ロゴや角印がぼやけるので、v1.x 候補の中でも効果が大きい。

**方針はこれまでと同じ「ブラウザに計算させる」**。SVG を自前でレイアウトしようとすると `viewBox` / `preserveAspectRatio` / 単位 / 継承の解決を全部書くことになるが、DOM 上の SVG なら次の 2 つで済む。

- **変換行列は `getScreenCTM()`**: 要素のユーザー単位からビューポート px への行列をブラウザが返してくれる。`viewBox` のスケールも入れ子の `transform` も込み。
- **塗りと線は `getComputedStyle()`**: プレゼンテーション属性（`fill="#08f"`）も CSS も継承も解決済みで、`fill` は `rgb(...)` / `none` / `url("#id")`、`stroke-width` は `3px`（= ユーザー単位）という素直な形で返る。幾何プロパティも computed style を優先すると `width="50%"` が `100px` に解決されて扱いやすい。

自前で書く必要があったのは**パスデータの正規化**だけ（`walker/svg-path.js`）。`d` を絶対座標の `M` / `L` / `C` / `Z` 列にし、`H` / `V` は `L` に、二次ベジェ（`Q` / `T`）は 3 次に、円弧（`A`）は仕様 F.6 の手順で中心・角度を求めて 90 度以下ごとの 3 次ベジェに分割する。基本図形も同じ列に変換する（角丸 rect と楕円はベジェ 4 区間）。

**座標変換**: パスの座標はユーザー単位のまま持ち、描画時に「ユーザー単位 → ドキュメント px」（`getScreenCTM()`）と「ドキュメント px → PDF pt・y 反転」を 1 つの行列に合成して `cm` で出す。こうすると線幅と破線もユーザー単位のまま書けて、CTM が自動で拡大してくれる（ブラウザと同じ挙動）。

**ExtGState の拡張**: SVG は塗りと線で不透明度が違いうるので、`gsName(ca, CA)` を別々に取れるようにした。

制約: `<text>` は警告して飛ばす（選択可能な文字として出すには回転や `textPath` の扱いが要るため、次の機会に）。`<use>` / `<image>` / `<foreignObject>`、paint server（`fill="url(#id)"`）も警告。`clip-path` / `mask` / `filter` は無視して図形をそのまま描く。

検証: パス・図形変換の単体テスト 24 件と、ブラウザテスト 2 件（円・破線角丸・円弧・多角形・折れ線・二次ベジェ・evenodd + transform を並べた SVG をブラウザ描画と画素差分。差分は曲線の縁のアンチエイリアスのみ。画像 XObject が生成されていないことも確認）。minify バンドルは gzip 21.8KB → 24.6KB。

## 23. GSUB によるグリフ置換（2026-09-20）

v1.x 候補に「GSUB（tabular-nums）」を挙げていたので、まず実現可能性を調べた。**結論として、tabular-nums は日本語フォントでは解決手段にならない**ことが分かったため、GSUB 対応の目的を置き換えて設計する。

### 調査でわかったこと

**1. 参照フォント（BIZ UDPGothic）に `tnum` が無い。** 持っている GSUB 機能は `aalt ccmp dlig dnom expt frac fwid hojo hwid jp78 jp83 jp90 liga nalt nlck numr ruby sups trad vert vkna vrt2 zero` で、`tnum` も `pnum` も `lnum` も `onum` も無い。Lookup の種類は 1（単一置換）・3（選択置換）・4（合字）だけ。

**2. ブラウザも何もしていない。** Chromium で `font-variant-numeric: tabular-nums` と `font-feature-settings: "tnum" 1` を当てて 100px で計測したが、`1111111111` = 629.89px、`8888888888` = 759.77px と指定の有無で変わらなかった。つまり**この件に関してブラウザと PDF の食い違いは無い**。

**3. それでも数字は揃わない。** BIZ UDPGothic の送り幅は `1` だけ 1290、他は 1556（unitsPerEm 2048）。プロポーショナル体なので当然で、GSUB では直せない。

**4. 他の日本語フォントも同じ。** Noto Sans JP / M PLUS 1p / IBM Plex Sans JP / Sawarabi Gothic / Zen Kaku Gothic New / BIZ UDGothic を調べたが、**どれにも `tnum` は無い**。ただし BIZ UDPGothic と Zen Kaku Gothic New 以外は数字の送り幅がもともと揃っている。

→ **領収証で金額の桁を揃えたいなら、フォントを選ぶのが正解**。プロポーショナル体（BIZ UD**P**Gothic）ではなく等幅系（BIZ UDGothic、送り幅は全数字 1024）を使うか、表のセルを右揃え + 列幅固定にする（これは今でも正しく出る）。ライブラリ側の実装で解決する話ではないので、ドキュメントで案内する。

### では GSUB 対応に意味はあるか

ある。`zero`（スラッシュ付きゼロ）で**実際に食い違いを再現できた**。`font-feature-settings: "zero" 1` を当てると、ブラウザはスラッシュ入りのゼロを描くのに、PDF は標準のゼロを描く（位置は実測なので合っている）。同じことが日本語帳票でより重要な機能でも起きる。

- `jp78` / `jp83` / `jp90` / `nlck` / `trad` / `expt` — **旧字体・異体字**。宛名の「髙橋」「﨑」などで効く
- `fwid` / `hwid` — 全角・半角形
- `zero` — スラッシュ付きゼロ
- `smcp` — スモールキャップス

いずれも **Lookup タイプ 1（単一置換）** で、1 文字 → 1 文字なので、1 文字ずつ位置を実測する現在のモデルと相性がよい。

### 設計案

**適用範囲**: Lookup タイプ 1（フォーマット 1 / 2）と、それを包むタイプ 7（拡張）のみ。タイプ 4（合字）はグリフ数が変わるため今のモデルでは扱えない（対応するなら測定単位を「文字」から「クラスタ」に変える大工事になる）。タイプ 3（選択置換）は選ぶ番号が要るので対象外。

**どの機能を適用するか**は computed style から決める。ブラウザが実際に有効にしている機能に合わせるのが原則。

| CSS | 機能タグ |
|---|---|
| `font-variant-east-asian: jis78 / jis83 / jis90 / jis04 / simplified / traditional / full-width / proportional-width / ruby` | `jp78` `jp83` `jp90` `jp04` `smpl` `trad` `fwid` `pwid` `ruby` |
| `font-variant-numeric: slashed-zero / tabular-nums / oldstyle-nums / …` | `zero` `tnum` `onum` … |
| `font-variant-caps: small-caps / all-small-caps` | `smcp` `c2sc` |
| `font-feature-settings: "xxxx" 1` | 指定タグ |

既定で有効な機能（`ccmp` / `liga` / `calt`）は**適用しない**。合字は扱えないし、勝手に変えるより現状維持の方が安全なため。

**実装**: `font/gsub.js` に ScriptList / FeatureList / LookupList のパーサを置き、要求された機能タグ集合から `gid → gid` の置換表を 1 つ作る（スクリプトは DFLT・latn・kana などを区別せず、同じタグの Lookup を合併する。実フォントではタグごとに同じ Lookup を指しているため）。`walker/text.js` が cmap で引いた gid にこの表を適用する。置換表は `(フォント, 機能集合)` でキャッシュする。サブセッタは置換後の gid をそのまま埋め込むので、**GSUB テーブル自体は PDF に入れない**（出力は増えない）。

**検証計画**: GSUB パーサの単体テストは実フォント（`fonts/BIZUDPGothic-Regular.ttf`）に対して行い、`zero` がゼロの gid を別の gid に写すこと、`fwid` が `yen` を全角に写すこと、`tnum` が存在しないことを確かめる。ブラウザテストは `font-feature-settings: "zero" 1` を当てた PDF とブラウザ描画の画素差分（現状は差分が出る。対応後に一致するはず）。

### 実装結果

設計どおりに実装した（`font/gsub.js`、`walker/text.js` から呼ぶ）。

- FeatureList を走査してタグ → Lookup 番号を作り、Lookup タイプ 1（フォーマット 1 / 2）とタイプ 7（拡張）だけを読む。ScriptList は見ずに全スクリプトの FeatureRecord を合併する（実フォントでは同じタグが同じ Lookup を指すため）。
- 複数機能を指定されたときは Lookup 番号の小さい順に合成する（後の Lookup は前の結果に適用される、という OpenType の規則に合わせる）。
- 置換結果は `(フォント, 機能集合)` ごとに WeakMap でキャッシュする。
- `cp`（ToUnicode 用のコードポイント）は置換前のまま持つので、**抽出テキストは元の文字**になる。スラッシュ付きゼロも「0」として検索・コピーできる。
- 壊れた GSUB は try/catch で「無いもの」として扱う。

検証: 実フォント（`fonts/BIZUDPGothic-Regular.ttf`）に対する単体テスト 13 件。`zero` がゼロの gid を別の gid に写すこと、`fwid` が半角円記号 `¥` を全角 `￥` に写すこと、`jp90` が異体字に写すこと、`tnum` が存在せず null になること、`liga`（タイプ 4）が読めず null になることを確かめた。`featureTagsOf` の CSS → タグ変換も含む。ブラウザテストは、同じ文字列を `slashed-zero` あり / なしで変換して PDF が変わること、抽出テキストは `0123` のままであること、ブラウザ描画との画素差分を検証する。

minify バンドルは gzip 24.6KB → 25.7KB。

**ドキュメントへの反映**: 数字の桁揃えは GSUB では解決しないため、`docs/css-support.md`・skill・`references/receipt-format.md` に「フォントを選ぶ」ことを明記した。とくに skill の領収証リファレンスには「BIZ UDPGothic は等幅」という**誤った記述があったので訂正**した（実際はプロポーショナル体で `1` だけ狭い）。

## 24. `background-repeat` 対応（2026-09-20）

**PDF のタイリングパターン（PatternType 1）を使わなかった理由**: パターン行列は「ページの既定座標系」への写像と定められていて、描画時の CTM を無視する。このライブラリは同じ描画命令を複数ページ・繰り返し `thead`・ヘッダー／フッターで使い回すので、パターンを使うと配置ごとに別オブジェクトを作る必要があり、シェーディングで使った「箱ローカル座標 + `cm`」の手が使えない。

**採った方法**: 走査の時点でタイル位置を計算し、**同じ画像を指す `image` 命令を必要な数だけ並べる**。画像 XObject は `image.key` で重複排除されるので、埋め込みは 1 回きり。命令が増えるだけで、コンテンツストリームは Flate 圧縮されるうえ繰り返しなのでよく縮む。ページ分割・クリップ・ヘッダーへの再利用も既存の仕組みがそのまま効く。新しい命令型も Painter の変更も要らなかった。

**軸ごとの計算**（`walker/image.js` の `splitRepeat` / `tileAxis`）: computed 値は `repeat-x` のような 1 値表記のまま返るので 2 値へ正規化する。並べる範囲は**描画領域（`background-clip` の箱）**で、位置決めの基準は**配置領域（`background-origin` の箱）**という CSS の区別に従う。

- `repeat`: 指定位置から前後へ伸ばす
- `round`: `n = max(1, round(領域 / タイル))` 個が収まるようタイルの大きさを `領域 / n` に変える
- `space`: `n = floor(領域 / タイル)` 個を等間隔に置き、余りを `n - 1` 個の隙間へ等分する。1 枚しか入らないなら先頭に 1 枚
- `no-repeat`: 従来どおり

**上限**: 小さなタイルを大きな面に敷くと命令数が爆発するので、4000 枚を超えたら 1 枚だけ描いて警告する（`background-size` を大きくするか、あらかじめタイル化した画像を使うよう案内する）。

検証: `splitRepeat` / `tileAxis` の単体テスト 13 件と、ブラウザテスト 2 件（6 モードを並べてブラウザ描画と画素差分。画像の埋め込みが 1 回だけであることも確認。上限超過の警告も検証）。

画素差分のテストでは最初、透過 PNG の角印を縮小して敷いたところ差分が 14% 出た。`no-repeat` の 1 枚にも同じ差分が出ていたことから、タイル配置ではなく**詳細画像を縮小したときのリサンプリング差**（Chromium と pdftoppm の違い）と判断し、28×28 を等倍で敷く単色タイルに替えた。Chromium では 2% 未満に収まった。

ただしこの閾値 2% は**締めすぎだった**。WebKit で 2.23% となり失敗した（他のブラウザテストはすべて通過）。タイルの位置は `space` の隙間 0.6px や `round` の 33.333px のように小数になるため、境界のアンチエイリアスがブラウザごとに違って当然で、数 % の差は正常。そこで検証の作りを変えた: **タイルの内側の色を点で見る決定的な検査**（`repeat-x` の右端は塗られている、縦には繰り返していない、など 9 点）を先に置き、全体の画素差分は他のテストと同じ 5% に緩めた。`splitRepeat` を常に `no-repeat` にする変異を入れて、この点検査が「repeat: 右下までタイルが並ぶ」という名前つきで失敗することを確認している。閾値を緩めても配置の退行は捕まえられる。

minify バンドルは gzip 25.7KB → 26.1KB。

## 25. ストリーミング出力の検討と、代わりに実装したもの（2026-09-20）

v1.x 候補に「ストリーミング出力」を挙げていたが中身を詰めていなかったので、まず**大きな文書で実際に何が起きるか**を測った。

### 測定

表の行数を変えて A4・余白 15mm・フッター付きで変換した（Chromium）。

| 行数 | ページ数 | 所要 | 出力 | ヒープ増 |
|---|---|---|---|---|
| 60 | 3 | 180ms | 20KB | 2.4MB |
| 600 | 約 26 | 572ms | 75KB | 10MB |
| 3000 | 約 130 | 2,750ms | 322KB | 45.5MB |

内訳（3000 行）: renderDocument（iframe のレイアウト）1,124ms、walk（走査と 1 文字ずつの実測）1,407ms、buildPdf（分割と PDF 組み立て）354ms。描画命令は 60,020 個、アトムは 15,006 個。

### 判断

**出力のストリーミングは見送る。** 130 ページで 322KB しかなく、`Blob` のままで困らない（`blob.stream()` もある）。PDF は xref が末尾なので逐次書き出し自体は可能だが、得られるものが無い。

**ページ単位の逐次生成（メモリ削減）も見送る。** ヒープの 45MB は主に全ページ分の DisplayList。これを捨てながら進めるには、全体を測り終える前に分割位置を確定する必要があり、「命令を動かさず、ページごとに描く範囲を決める」という土台（`thead` / `tfoot` の繰り返しや `break-*: avoid` が素直に書けている理由）に手が入る。割に合わない。

**実際の痛みはメインスレッドの占有だった**ので、そこを直した。

### 実装 1: 進捗通知とイベントループへの譲渡

`src/pacer.js` に時間ベースの譲渡を置いた。最後に譲ってから 12ms を超えていたら 1 回だけイベントループへ戻す。`scheduler.yield()` があればそれを、無ければ MessageChannel を使う（`setTimeout(0)` はネストすると 4ms にクランプされるため）。小さな文書ではほとんど発火しない。

譲る場所は `visit()` の先頭（走査中）と、ページ描画ループ・画像埋め込みループ。`onProgress` で `render` → `walk` → `layout`（総ページ数が確定）→ `page`（1 ページずつ）→ `done` を通知する。

効果（3000 行、走査フェーズだけを切り出して requestAnimationFrame で計測）:
...
変換全体でもフレームは届くようになり（1,115ms で 24 フレーム）、最長のフレーム間隔は 181ms まで下がった。残る 181ms は `renderDocument` の 1 回のレイアウトで、これは分割できない。

### 実装 2: 埋め込み後のピクセルデータ解放

`walker/image.js` はデコード結果（RGB・アルファ・JPEG のバイト列）を URL キーのモジュールキャッシュに**永久に**持っていた。同じロゴを何度も変換するときは効くが、画像の多い文書では際限なく積み上がる。

`releasePixels()` を足し、`embedImage()` の直後にピクセルデータを捨ててキャッシュからも外すようにした。**速度よりメモリを優先する**判断で、次の変換では読み直しになる（並行する別の変換が先に解放していた場合に備えて、`buildPdf` 側にも読み直しの経路を用意した）。

効果（600×600 の透過 PNG を 12 枚）: 変換中のピーク +34.4MB、**変換後に残るのは 1.3MB**。以前はデコード済みの 12 枚（約 17MB）がページを開いている間ずっと残っていた。ピークが下がりきらないのは、走査時に全部デコードして埋め込み時にまとめて解放しているため。埋め込み直前までデコードを遅らせればさらに下がるが、それは走査の構造に手が入るので見送った。

minify バンドルは gzip 26.1KB → 26.4KB。

## 26. リンク注釈としおり（2026-09-20）

`<a href>` はこれまでただの文字として描かれるだけで、PDF 上ではクリックできなかった。PDF のリンク注釈（`/Annot /Link`）としおり（`/Outlines`）を足した。

**描画命令にしない**: リンクもしおりも「どこに何があるか」であって描画ではない。DisplayList に入れると transform グループやクリップの入れ子に巻き込まれ、Painter が座標を二重に変換してしまう。そこで `WalkResult` に `links` / `anchors` / `headings` を別立てで持たせた。`getClientRects()` は transform 適用後の矩形を返すので、変形の中のリンクもそのまま外接矩形として扱える（PDF の注釈は軸並行の矩形しか持てないので、これが妥当な近似）。

**折り返しとページ跨ぎ**: インラインリンクは行ごとに矩形が返るので、そのぶん注釈を作る。ページ境界を跨ぐリンクは、ページ範囲 `[start, end)` で上下を切り取ってから用紙座標へ写し、両方のページに注釈を出す。

**文書内リンク**: `href="#id"` は走査時には解決できない（どのページに載るかが未確定）。`anchors`（id → ドキュメント y）を集めておき、ページ範囲が確定してから `locate()` でページと位置を求める。飛び先は前後どちらのページにもなりうるので、**ページオブジェクトの参照を先に `reserve()` しておく**のがポイント。飛び先の無い `#id` と `javascript:` は注釈にしない。

**ヘッダー／フッター**: 本文とは別の座標系（帯の上端と docTop 0）なので、切り取りと変換をまとめた `addLinks(list, pdfTop, docTop, docEnd)` を作って 3 か所から呼ぶ形にした。

**しおり**: 見出しの並びをレベルで木に組み直し、`/First` `/Last` `/Next` `/Prev` `/Parent` `/Count` をつないで書き出す。`Count` を正にすると既定で開いた状態になる。カタログに `/PageMode /UseOutlines` を入れてサイドバーを開かせる。既定は作らない（`outline: true` で有効）。

検証: ブラウザテスト 6 件。pdf.js の `getAnnotations()` / `getOutline()` / `getPageIndex()` で、外部 URL・`mailto:`・文書内リンクの飛び先ページ、`javascript:` と飛び先なしを除外していること、`links: false` で作らないこと、ページ跨ぎで 2 つに分かれて高さの合計が元に戻ること、フッターのリンクが全ページに出ること、しおりが `h1 > h2 > h3` に入れ子になることを確かめた。さらに、抽出した文字の描き出し位置が注釈の矩形の中にあることも見ている（y 反転の取り違えを捕まえるため）。

minify バンドルは gzip 26.4KB → 27.6KB。
