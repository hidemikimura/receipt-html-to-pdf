...
→ このうち `break-before/after: avoid`（20 章）・グラデーション（21 章）・SVG（22 章）・GSUB（23 章）・`background-repeat`（24 章）は対応済み。ストリーミング出力は 25 章の測定で見送り、代わりに進捗通知とメモリ解放を実装した。リンク注釈としおりも 26 章で対応済み。**残り: 縦書き。**
...
検証: 単体テスト 6 件（発火・非発火・連鎖・諦める 2 条件）とブラウザテスト 1 件（46 行 + 見出し + 段落で、avoid 無しなら見出しが 1 ページ目末尾に残り、avoid 有りなら 2 ページ目へ段落と一緒に送られる）。minify バンドルは gzip 19.9KB → 20.3KB。
