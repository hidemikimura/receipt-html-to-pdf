...
- 作成者: 木村英実（ecx Inc.）
- ステータス: **0.3.0 リリース準備中**（0.1.0 / 0.2.0 / 0.2.1 を公開済み）。API は 1.0.0 まで変更の可能性あり
- ライブラリ名: **Receipt html to pdf**
- npm パッケージ名: `@hidemikimura/receipt-html-to-pdf` — 0.1.0（2026-09-18）、0.2.0（シャドウ DOM・18 章）、0.2.1（右端が切れる不具合の修正・19 章）を公開済み。次は 0.3.0（20〜25 章）
...
→ このうち `break-before/after: avoid`（20 章）・グラデーション（21 章）・SVG（22 章）・GSUB（23 章）・`background-repeat`（24 章）は対応済み。ストリーミング出力は 25 章の測定で見送り、代わりに進捗通知とメモリ解放を実装した。**残り: 縦書き、リンク注釈。**
